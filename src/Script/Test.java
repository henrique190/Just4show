	...
	public void checkEmail() {
		...
		try {
			if (inbox.getUnreadMessageCount() > messageCount) {
				...
				if(urlCatch.contains("m=email-register")) {
					System.out.println("Estou em email");
					ValidadeNewEmail validadeNewEmail = new ValidadeNewEmail(urlCatch);
					validadeNewEmail.action();
					Configs.hasNewEmail = true;
					
				}
				
				
				
				if(urlCatch.contains("m=password")) {
					System.out.println("Estou em password");
					ChangePassword changePassword = new ChangePassword(urlCatch, Configs.newPassword);
			...
			System.out.println("Erro ao receber novo email2: "+e);
		}
	}
